...
.cf-input{border:none;border-bottom:1.5px solid var(--rule);background:transparent;font-family:var(--sans);font-size:.95rem;padding:.5rem 0;color:var(--ink);outline:none;transition:border-color .15s;width:100%}
.cf-input:focus{border-bottom-color:var(--ink)}
.cf-textarea{border:none;border-bottom:1.5px solid var(--rule);background:transparent;font-family:var(--sans);font-size:.95rem;padding:.5rem 0;color:var(--ink);outline:none;resize:none;width:100%;min-height:80px;line-height:1.6;transition:border-color .15s;overflow:hidden}
.cf-textarea:focus{border-bottom-color:var(--ink)}
.comment-submit{background:var(--ink);color:#f7f5f0;border:none;font-family:var(--sans);font-size:.78rem;font-weight:500;letter-spacing:.08em;text-transform:uppercase;padding:.75rem 2rem;cursor:pointer;transition:background .15s;margin-top:1.25rem}
.comment-submit:hover{background:var(--accent)}
.comment-submit:disabled{opacity:.4;cursor:not-allowed}
.cf-msg{font-size:.82rem;margin-top:.6rem;min-height:1.2em;display:block}
  ...
  window.submitComment = function() {{
    ...
    .then(function(r) {{
      if (r.ok || r.status === 201) {{
        ...
        msg.className = 'comment-form-msg ok';
        ...
        msg.className = 'comment-form-msg err';
      }}
    }})
    .catch(function() {{
